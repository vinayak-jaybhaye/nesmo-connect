...
function NewOpportunity({ onNewOpportunity }) {
  ...
  const handleOpportunityPost = async () => {
    ...
    try {
      const response = await dbServices.createOpportunity(newOpportunity);
      newOpportunity.id = response.id;
      newOpportunity.createdAt = response.createdAt;
      newOpportunity.createdBy = { ...user, id: user.uid };
  ...
  return (
    <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 p-5 rounded-2xl shadow-2xl border border-gray-700/50 backdrop-blur-sm">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div
          className="p-2 bg-green-500/20 rounded-xl cursor-pointer"
